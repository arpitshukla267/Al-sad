...
const FeaturedArticle = () => {
	...
	return (
		<div className="flex flex-col lg:grid lg:grid-cols-2 gap-6 lg:gap-[48px] mb-12 sm:mb-16">
			...
			<div className="order-2 lg:order-1 flex flex-col justify-center space-y-3 lg:space-y-0">
				...
				<div className="flex flex-col gap-4 lg:gap-10 mt-4 lg:mt-0">
					<Link href={`/blogs/${featured.id}`}>
						<h2 className="text-[24px] sm:text-[32px] lg:text-[40px] font-primary font-bold text-[#0e2143] leading-[140%] line-clamp-3 hover:underline">
							{featured.title}
						</h2>
					</Link>
				<Link
			...
			{/* Image */}
			<Link href={`/blogs/${featured.id}`} className="order-1 lg:order-2 relative w-full h-[372px] sm:h-[463px] lg:h-[463px] rounded-[4px]">
				<Image
					src={featured.image}
					alt={featured.title}
					fill
